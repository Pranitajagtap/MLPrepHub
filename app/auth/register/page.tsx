...
export default function Register() {
  ...
  useEffect(() => {
    // Check if user has guest onboarding data
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Trigger auth event to notify FloatingProfile
      window.dispatchEvent(new CustomEvent('authChange', { 
        detail: { user: data.user, action: 'login' } 
      }))

      // Check if user has guest onboarding data to transfer
      const guestData = localStorage.getItem('guestOnboarding')
      if (guestData) {
        console.log('🎉 Found guest onboarding data, transferring...')
        ...
        try {
          ...
          const onboardingResponse = await fetch('/api/onboarding', {
            ...
            body: JSON.stringify({
              userId: data.user.id,
              guestOnboardingData: JSON.parse(guestData)
            }),
          })
          ...
          if (onboardingResponse.ok) {
            ...
            console.log('⚠️ Onboarding save failed, redirecting to onboarding')
            router.push('/onboarding')
          }
        } catch (onboardingError) {
          console.error('❌ Onboarding transfer error:', onboardingError)
          router.push('/onboarding')
        }
      } else {
        console.log('➡️ No guest data - redirecting to onboarding')
        router.push('/onboarding')
      }
          ...
          {hasGuestOnboarding && (
            <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg">
              <p className="text-green-700 text-sm">
                🎉 We found your career matches! They'll be saved to your account.
              </p>
            </div>
          )}
        </div>
